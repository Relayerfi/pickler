import { createAgentQueries, createAuthenticateAgent, createAuthenticateRequest, createProfileService, parseEmailAllowlist } from "@pickler/core";
import {
  createSupabaseAdmin,
...
function required<K extends "SUPABASE_URL" | "SUPABASE_SECRET_KEY" | "JWT_ISSUER" | "ENCRYPTION_KEY">(env: Env, name: K): string {
...
export function createServices(env: Env): Services {
  const db = createSupabaseAdmin({ url: required(env, "SUPABASE_URL"), secretKey: required(env, "SUPABASE_SECRET_KEY") });
  ...
  return {
    authenticate: createAuthenticateRequest({
      tokens: createSupabaseJwtVerifier({ jwksUrl: `${issuer}/.well-known/jwks.json`, issuer, ...(env.JWT_AUDIENCE ? { audience: env.JWT_AUDIENCE } : {}) }),
      workspaces,
