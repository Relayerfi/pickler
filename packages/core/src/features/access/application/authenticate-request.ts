...
import { AccessDeniedError, AuthenticationRequiredError, InactiveWorkspaceError } from "../domain/errors";
import { isIpAllowed } from "../domain/ip-allowlist";
import { buildApiKeyPrincipal, buildUserPrincipal, type Principal } from "../domain/principal";
import { InvalidAccessTokenError, type AccessTokenVerifier } from "../ports/access-token-verifier";
import type { ApiKeyDirectory } from "../ports/api-key-directory";
import type { Workspace, WorkspaceDirectory } from "../ports/workspace-directory";
...
export function createAuthenticateRequest(deps: AuthenticateDependencies) {
  async function viaToken(token: string, selectedWorkspaceId: string | null): Promise<Authenticated | null> {
    let user;
    try {
      user = await deps.tokens.verify(token);
    } catch (error) {
      if (error instanceof InvalidAccessTokenError) return null;
      throw error;
    }
    ...
    if (workspace && workspace.userId === user.userId) role = "admin";

    if (selectedWorkspaceId) {
      const selectedRole = await deps.workspaces.roleOf(user.userId, selectedWorkspaceId);
      if (!selectedRole) throw new AccessDeniedError("Not a member of the requested workspace");
    ...
    if (workspace && !workspace.isActive) throw new InactiveWorkspaceError();
  ...
  async function viaApiKey(rawKey: string, clientIp: string): Promise<Authenticated | null> {
    ...
    if (!key || !key.active || !key.workspaceId) return null;
    if (key.expiresAt && key.expiresAt.getTime() <= now) return null;
    if (key.linkExpiresAt && key.linkExpiresAt.getTime() <= now) return null;
    if (!isIpAllowed(clientIp, key.allowedCidrs)) throw new AccessDeniedError("IP address not in allowlist for this API key");

    const workspace = await deps.workspaces.findById(key.workspaceId);
    if (!workspace) return null;
    if (!workspace.isActive) throw new InactiveWorkspaceError();
    return { principal: buildApiKeyPrincipal({ id: key.id, scopes: key.scopes }, workspace.id), workspace };
  }

  return async (credentials: RequestCredentials, options: AuthenticateOptions = {}): Promise<Authenticated> => {
    if (credentials.bearerToken) {
      const result = await viaToken(credentials.bearerToken, credentials.selectedWorkspaceId);
      if (result) return result;
    }
    if (options.userOnly) throw new AuthenticationRequiredError("Bearer JWT required");
