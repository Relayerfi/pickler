...
function parseIpv6(ip: string): bigint | null {
  ...
  const groups = [...headGroups, ...Array<string>(rest === undefined ? 0 : missing).fill("0"), ...tailGroups];
  let value = 0n;
...
function matches(address: Address, cidr: string): boolean {
  ...
  if (!base || prefixText === undefined || extra !== undefined || !/^\d{1,3}$/.test(prefixText)) return false;
  const network = parseAddress(base);
  if (!network || network.family !== address.family) return false;
