...
import { createJoinWaitlist, InvalidEmailError, type Applicant, type ApplicantRepository, type WaitlistRepository } from "../src/index.ts";

const applicant = (email: string): Applicant => ({ email, linePosition: 10, referralCode: "abc12345", referrals: 0, application: null });
...
test("waitlist stores the normalized email and a valid referral code", async () => {
  ...
  assert.deepEqual(await join("  Creator@Example.COM ", { referralCode: " AbC12345 " }), { position: 1205, alreadyJoined: false, applyToken: "token-1" });
...
test("waitlist rejects invalid emails without touching the repository", async () => {
  ...
  for (const email of ["", "not-an-email", "a@b", "two words@example.com", `${"x".repeat(250)}@example.com`]) {
...
test("a repeat signup gets the apply token back only from the browser that holds it", async () => {
  const waitlist: WaitlistRepository = { join: async () => ({ position: 7, alreadyJoined: true, applyToken: null }) };
  const join = createJoinWaitlist(waitlist, applicantsWith({ mine: applicant("me@example.com"), theirs: applicant("them@example.com") }));
