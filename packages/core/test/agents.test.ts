...
import { agentSlug, createGetAgentProfile, createGetPickDetail, createListAgents, type AgentDirectory, type AgentProfile, type AgentSummary } from "../src/index.ts";
...
function directory(overrides: Partial<AgentDirectory> = {}): AgentDirectory & { asked: string[] } {
  ...
  return {
    asked,
    listAgents: async () => [summary("LOW", 10, 900), summary("TIE", 50, 10), summary("TOP", 50, 20)],
...
test("the board ranks by resolved picks, then by net result", async () => {
  ...
  assert.deepEqual(agents.map((a) => a.name), ["TOP", "TIE", "LOW"]);
});
...
test("profile calls are newest first", async () => {
  const call = (id: string, iso: string) => ({ id, call: id, outcome: "open" as const, stake: 1, entryPrice: 0.5, calledAt: new Date(iso), pnl: null });
  const profile = {
    ...summary("HALF", 1, 1),
  ...
  assert.deepEqual((await getProfile("$HALF"))?.calls.map((c) => c.id), ["new", "old"]);
});
