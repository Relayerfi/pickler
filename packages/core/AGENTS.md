...
The research runner requires only the `agent`, `event` and `finish` repository methods. Admission, claims and process ownership remain responsibilities of its host; narrowing this dependency does not make the runner durable or permit bypassing production authorization.
