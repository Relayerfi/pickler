...
`src/trading.ts` defines strict manual purchase requests and public order/account responses. Credentials, signed payloads and leases are excluded. `costUpperBound` is a conservative authorization bound, not actual fees paid. Agent config optionally includes `trading: { version: 1, mode: "off" | "manual" | "automatic" }` and the independent `polymarket-trading` plugin; historical omissions stay off. Research decisions still contain no executable sizes.
