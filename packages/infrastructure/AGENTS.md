...
`PostgresTradingStore` and migration 0005 implement private trading accounts, durable order intents, one manual/agent slot, shared budget admission and unsent leases. `PolymarketTrading` pins the official SDK, requires an already deployed Deposit Wallet, and never bootstraps approvals at runtime. Store the deterministic order hash before posting; never retry an ambiguous write with a replacement order. Confirmation requires provider-confirmed trades plus the wallet position. Node-only filesystem/setup helpers use the separate `@pickler/infrastructure/wallet` and `/trading-operator` exports; keep them out of the default barrel/Cloudflare bundle. Tests inject deterministic public fixture keys, SDK clients and isolated databases; never fund or operate real wallets in tests.
