...
const decodeAgent = (row: Row): AgentRecord => ({
  ...
  config: JSON.parse(String(row.config)) as AgentConfig,
  paused: Boolean(row.paused),
  scheduleEnabled: Boolean(row.schedule_enabled),
  nextDueAt: row.next_due === null ? null : Number(row.next_due),
});
const decodeRun = (row: Row): RunRecord => ({
  ...
  config: JSON.parse(String(row.config)) as AgentConfig,
  decision: row.decision === null ? null : (JSON.parse(String(row.decision)) as Decision),
...
export class SqliteResearchStore implements ResearchRepository {
  ...
  private begin() {
    return this.retryBusy(() => this.client.transaction("write"));
  }

  private exec(input: InStatement) {
    return this.retryBusy(() => this.client.execute(input));
  }
  ...
  async bindConnectionIdentity(identity: string): Promise<void> {
    ...
    try {
      const previous = (
        await tx.execute("SELECT value FROM metadata WHERE key='connection_identity'")
      ).rows[0]?.value;
      if (previous !== identity) {
        await tx.execute(
          "INSERT INTO metadata(key,value) VALUES('connections_checked','false') ON CONFLICT(key) DO UPDATE SET value='false'",
        );
        await tx.execute({
          sql: `
          INSERT INTO metadata(key, value)
          VALUES('connection_identity', ?)
          ON CONFLICT(key)
          DO UPDATE
        ...
        await tx.execute(
          "UPDATE runs SET status='cancelled',error='CONNECTIONS_CHANGED' WHERE status='queued' AND trigger='schedule'",
        );
      }
      await tx.commit();
    } finally {
  ...
  async agents(tenantId: string): Promise<AgentRecord[]> {
    return (
      await this.exec({
        sql: "SELECT * FROM agents WHERE tenant_id=? ORDER BY id",
        args: [tenantId],
      })
    ).rows.map(decodeAgent);
  }

  private async getAgent(
    scope: Scope,
    db: Client | Transaction = this.client,
  ): Promise<AgentRecord> {
    const row = (
      await db.execute({
        sql: "SELECT * FROM agents WHERE tenant_id=? AND id=?",
        args: [scope.tenantId, scope.agentId],
      })
    ).rows[0];
    if (!row) {
  ...
  async updateConfig(
    ...
    const result = await this.exec({
      sql: `
          UPDATE agents
          SET config=?, version=version+1, schedule_enabled=0, next_due=NULL
          WHERE tenant_id=?
          AND id=?
          AND version=?
        `,
  ...
  async pause(scope: Scope, paused: boolean): Promise<void> {
    ...
    try {
      ...
      await tx.execute({
        sql: "UPDATE agents SET paused=? WHERE tenant_id=? AND id=?",
        args: [Number(paused), scope.tenantId, scope.agentId],
      });
      if (paused) {
        await tx.execute({
          sql: `
  ...
  async schedule(scope: Scope, enabled: boolean, now: number): Promise<void> {
    ...
    try {
      ...
      if (enabled) {
        const checked =
          (await tx.execute("SELECT value FROM metadata WHERE key='connections_checked'")).rows[0]
            ?.value === "true";
        const success =
          (
            await tx.execute({
          ...
          LIMIT 1
        `,
              args: [scope.tenantId, scope.agentId, agent.version],
            })
          ).rows.length > 0;
        assertCanSchedule(agent, checked, success);
      }
      await tx.execute({
        sql: "UPDATE agents SET schedule_enabled=?,next_due=? WHERE tenant_id=? AND id=?",
        args: [
          Number(enabled),
          enabled ? nextOccurrence(agent, now) : null,
          scope.tenantId,
          scope.agentId,
        ],
      });
      if (!enabled) {
        await tx.execute({
          sql: `
          UPDATE runs
          SET status='cancelled', error='SCHEDULE_DISABLED', finished_at=?
          WHERE tenant_id=?
          AND agent_id=?
          AND status='queued'
          AND trigger='schedule'
        `,
          args: [now, scope.tenantId, scope.agentId],
        });
      }
      await tx.commit();
    } finally {
  ...
  private async insertRun(
    ...
    const old = (
      await tx.execute({
        sql: "SELECT * FROM runs WHERE tenant_id=? AND agent_id=? AND request_key=?",
        args: [agent.tenantId, agent.id, key],
      })
    ).rows[0];
    if (old) {
      const run = decodeRun(old);
      if (run.marketId !== marketId) {
    ...
    const count = Number(
      (
        await tx.execute({
          sql: "SELECT COUNT(*) AS n FROM runs WHERE tenant_id=? AND agent_id=? AND created_at>?",
          args: [agent.tenantId, agent.id, now - DAY],
        })
      ).rows[0]?.n,
    );
    ...
    await tx.execute({
      sql: `
          INSERT INTO runs(id, tenant_id, agent_id, request_key, market_id, trigger, status, created_at, config_version, config)
          VALUES(?, ?, ?, ?, ?, ?, 'queued', ?, ?, ?)
        `,
      args: [
        id,
    ...
    return decodeRun(
      (await tx.execute({ sql: "SELECT * FROM runs WHERE id=?", args: [id] })).rows[0]!,
    );
  }
  ...
  async run(tenantId: string, runId: string): Promise<RunRecord> {
    const row = (
      await this.exec({
        sql: "SELECT * FROM runs WHERE tenant_id=? AND id=?",
        args: [tenantId, runId],
      })
    ).rows[0];
    if (!row) {
  ...
  async events(tenantId: string, runId: string): Promise<RunEvent[]> {
    ...
    return (
      await this.exec({ sql: "SELECT * FROM events WHERE run_id=? ORDER BY id", args: [runId] })
    ).rows.map((r) => ({
      id: Number(r.id),
      type: String(r.type),
      data: JSON.parse(String(r.data)) as unknown,
  ...
  async event(run: RunRecord, type: string, data: unknown, now: number): Promise<void> {
    ...
    await this.exec({
      sql: "INSERT INTO events(run_id,type,data,created_at) VALUES(?,?,?,?)",
      args: [run.id, type, JSON.stringify(data) ?? "null", now],
    });
  }
  ...
  async claim(now: number): Promise<RunRecord | null> {
    ...
    try {
      await tx.execute({
        sql: `
          UPDATE runs
          SET status='cancelled', error='CONFIG_CHANGED', finished_at=?
          WHERE status='queued'
            AND EXISTS (
              SELECT 1
              FROM agents a
              WHERE a.tenant_id=runs.tenant_id
                AND a.id=runs.agent_id
                AND a.version<>runs.config_version
            )
        `,
        args: [now],
      });
      ...
      await tx.execute({
        sql: "UPDATE runs SET status='running',started_at=? WHERE id=?",
        args: [now, String(row.id)],
      });
      await tx.commit();
      return { ...decodeRun(row), status: "running", startedAt: now };
    } finally {
      tx.close();
    }
  }
  ...
  async finish(
    ...
    await this.exec({
      sql: `
          UPDATE runs
          SET status=?, decision=?, error=?, finished_at=?
          WHERE id=?
          AND tenant_id=?
          AND status='running'
        `,
  ...
  async tick(now: number): Promise<void> {
    ...
    try {
      const due = (
        await tx.execute({
          sql: "SELECT * FROM agents WHERE schedule_enabled=1 AND paused=0 AND next_due<=?",
          args: [now],
        })
      ).rows;
      for (const row of due) {
        ...
        await tx.execute({
          sql: "UPDATE agents SET next_due=? WHERE tenant_id=? AND id=?",
          args: [nextOccurrence(agent, now), agent.tenantId, agent.id],
        });
      }
      await tx.commit();
    } finally {
