  ...
  return {
    async loadBudgets(agentId) {
      const { data, error } = await db
        ...
        .eq("agent_id", agentId)
        .returns<{ category: string; limit_micro_usd: string | number; spent_micro_usd: string | number }[]>();
      if (error) throw new DataSourceUnavailableError("supabase.budget.budgets", { cause: new Error(error.message) });
      return (data ?? []).map((row) => ({ category: row.category, limit: String(row.limit_micro_usd), spent: String(row.spent_micro_usd) }));
    },
  };
}
