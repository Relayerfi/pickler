...
const abstention: Decision = { action: 'ABSTAIN', marketId: '1', outcomeId: null, thesis: 'Test evidence', counterEvidence: 'Opposing evidence', uncertainty: 'Uncertain', sourceIds: ['test-source'], estimatedProbability: null, observedPrice: null, limitPrice: null, expiresAt: null, abstentionReason: 'Insufficient evidence' };
async function setup(t: TestContext) {
  const dir = await mkdtemp(join(tmpdir(), 'pickler-store-'));
  const store = new SqliteResearchStore(`file:${join(dir, 'test.db')}`); await store.init();
  t.after(async () => { store.close(); await rm(dir, { recursive: true, force: true }); });
  await store.updateConfig(scope, 1, config); return store;
}
test('tenant scope protects agents, configuration, runs and evidence', async t => {
  ...
  await assert.rejects(store.updateConfig({ ...scope, tenantId: 'beta' }, 2, config), { code: 'NOT_FOUND' });
...
test('idempotency, concurrency, rolling quota and version conflicts', async t => {
  const store = await setup(t);
  const runs = await Promise.all([store.enqueue(scope, 'same', null, 1000), store.enqueue(scope, 'same', null, 1000)]);
  ...
  for (let i = 2; i < 6; i++) await store.enqueue(scope, `q-${i}`, null, 1003);
...
test('schedule readiness, coalescing, deduplication, pause and recovery use a controlled clock', async t => {
  ...
  await store.tick(due + 9 * 3_600_000); await store.tick(due + 9 * 3_600_000);
...
test('provider identity changes invalidate readiness, but ordinary restarts preserve it', async t => {
  ...
  await store.setConnectionsChecked(true);
  await store.enqueue(scope, 'manual-ready', null, 0); const run = (await store.claim(1))!;
  await store.finish(run, abstention, null, 2); await store.schedule(scope, true, 3);
  await store.bindConnectionIdentity('first'); assert.equal((await store.agent(scope)).scheduleEnabled, true);
  await store.bindConnectionIdentity('second'); assert.equal((await store.agent(scope)).scheduleEnabled, false);
