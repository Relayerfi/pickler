...
import { createSupabaseAdmin, createSupabaseProfileRepository } from "../src/index.ts";
...
function fake(replies: { status?: number; body: unknown }[]) {
  ...
  return { repo: createSupabaseProfileRepository(createSupabaseAdmin({ url: "https://ref.supabase.co", secretKey: "sb_secret_x", fetch: fetchFn })), requests };
}

const row = { user_id: "u1", display_name: "Ana Robles", handle: "anarobles", created_at: "2026-09-16T12:00:00+00:00" };
...
test("create registers handle and profile through identity.create_profile, then reads the profile", async () => {
  ...
  const result = await repo.create({ userId: "u1", displayName: "Ana Robles", handle: "anarobles" });
  ...
  assert.deepEqual(await requests[0]!.json(), { p_user_id: "u1", p_display_name: "Ana Robles", p_handle: "anarobles" });
...
test("create maps function outcomes and rejects unknown ones", async () => {
  const { repo } = fake([{ body: "handle_taken" }, { body: "user_has_profile" }, { body: "maybe" }]);
