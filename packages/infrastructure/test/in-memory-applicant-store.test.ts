...
import { createInMemoryApplicantStore } from "../src/index.ts";

const application = { agentName: "Pick", ticker: "$PICK", xHandle: "@pickbot", category: "Sports", personality: "Analyst", edge: "e", whyYou: "w" } as const;

test("in-memory store issues a token once and counts referrals that applied", async () => {
  const { waitlist, applicants } = createInMemoryApplicantStore({ initialCount: 1204, reservedTickers: ["$HALF"] });
  ...
  assert.equal(await applicants.submit(bo.applyToken!, { ...application, ticker: "$HALF" }), "ticker_taken");
  ...
  assert.equal(await applicants.submit(ana.applyToken!, { ...application, ticker: "$ANA", xHandle: "@PICKBOT" }), "handle_taken");
