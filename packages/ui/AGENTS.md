...
`@pickler/ui` contains product-independent React components and the control system both surfaces share. Current files are `src/button.tsx`, `src/chart.tsx`, `src/chip.tsx`, `src/icon.tsx`, `src/meter.tsx`, `src/index.ts`, and `src/styles.css`. The package exports `Button`, `ButtonLink`, `CandleChart`, `BarChart`, `Chip`, `Tag`, `Icon`, `Meter` and the stylesheet `@pickler/ui/styles.css`. The web root layout imports that stylesheet.
...
- New controls use these components, and the app is expected to have no hand-rolled button, chip or bar left. Feature stylesheets may keep their own layout, but not their own shapes.
