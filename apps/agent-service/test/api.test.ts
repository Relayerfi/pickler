...
test('HTTP authorization, scoped writes, async run dispatch and polling', async t => {
  ...
  const unavailable = async () => { throw new Error('Not used in this test'); };
  const markets: MarketData = { categories: async () => [], list: unavailable, get: unavailable, book: unavailable };
  const app = createApi({ repository, markets, tokens: { alpha: 'a'.repeat(32), beta: 'b'.repeat(32) }, checkConnections: unavailable });
  const request = (path: string, tenant = 'a', method = 'GET', body?: unknown) => app.request(path, { method, headers: { Authorization: `Bearer ${tenant.repeat(32)}`, 'Content-Type': 'application/json', 'Idempotency-Key': 'test-key' }, ...(body !== undefined ? { body: JSON.stringify(body) } : {}) });
  ...
  assert.equal((await request('/agents/pickle-alpha/config', 'a', 'PUT', { ...update, tenantId: 'beta' })).status, 400);
  ...
  assert.equal((await request('/agents/pickle-alpha/pause', 'b', 'PUT', { paused: true })).status, 404);
});
test('closed tool registry omits disabled capabilities and validates output', async () => {
  ...
  const invalid = await tools.searchWeb!.inputSchema!['~standard'].validate({ query: '', tenantId: 'beta' });
  assert.ok(invalid.issues?.length);
  await assert.rejects(async () => tools.searchWeb!.execute!({ query: 'test', intent: 'supporting' }, {} as never));
