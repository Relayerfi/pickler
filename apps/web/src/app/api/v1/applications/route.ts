import { APPLICATION_LIMITS_DTO, type ApplicantResponse, type ApplicationFieldDto } from "@pickler/api-schema";
...
const FIELDS: ApplicationFieldDto[] = ["agentName", "ticker", "xHandle", "category", "personality", "edge", "whyYou"];
// Generous ceiling above the business limits; exact lengths are checked in core.
const MAX_BODY_BYTES = 4 * (APPLICATION_LIMITS_DTO.agentName + APPLICATION_LIMITS_DTO.edge + APPLICATION_LIMITS_DTO.whyYou) + 1024;

export async function GET() {
  try {
    const applicant = await services.getApplicant(await readApplyToken());
    if (!applicant) return errorResponse(401, "no_seat", "Join the waitlist from this browser first.");
...
export async function POST(request: Request) {
  ...
  for (const field of FIELDS) {
    ...
    if (typeof value !== "string") return errorResponse(400, "invalid_request", `Field '${field}' must be a string.`);
    input[field] = value;
  }
  ...
  try {
    ...
    return Response.json(response, { status: 201, headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
    return toErrorResponse(error);
  }
}
