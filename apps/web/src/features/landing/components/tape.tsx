...
export function Tape() {
  ...
  const items = (copy: boolean) =>
    tape.map((entry, i) => (
      <a key={`${copy ? "b" : "a"}-${i}`} href="#calls" className={styles.tapeItem} aria-hidden={copy || undefined} tabIndex={copy ? -1 : undefined}>
        <span className={styles.tapeTicker}>{entry.ticker}</span>
        <span className={styles.tapeCall}>{entry.call}</span>
        <span className={`${styles.tapeResult} ${toneClass[entry.outcome]}`}>{formatTapeResult(entry.outcome, entry.stake, entry.pnl)}</span>
      </a>
    ));
