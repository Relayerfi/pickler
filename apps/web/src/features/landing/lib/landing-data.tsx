...
export function LandingDataProvider({ initial, children }: { initial: LandingResponse; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (response.ok) setData((await response.json()) as LandingResponse);
      } catch {
...
export function useLanding(): LandingState {
  ...
  if (!value) throw new Error("useLanding must be used inside LandingDataProvider");
  return value;
}
